...
class TestWeatherDataService(unittest.TestCase):
    ...
    @patch('app.Producer')
    def test_fetch_and_publish_storm_reports_noaa(self, mock_producer, mock_get):
        ...
        mock_producer.return_value.produce = mock_produce
    ...
    @patch('app.Producer')
    def test_fetch_and_publish_storm_reports_local(self, mock_producer, mock_open):
        # Set the USE_LOCAL_TEST flag to True
        with patch.dict('app.config', {'test': {'flag': True, 'csv': './../today.csv'}}):
            # Mock the Kafka producer
            mock_produce = MagicMock()
            mock_producer.return_value.produce = mock_produce
